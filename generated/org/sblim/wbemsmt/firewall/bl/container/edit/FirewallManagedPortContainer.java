/** 
  * FirewallManagedPortContainer.java
  *
  * 
  * Â© Copyright IBM Corp.  2009,2006,2007
  *
  * THIS FILE IS PROVIDED UNDER THE TER	MS OF THE ECLIPSE PUBLIC LICENSE
  * ("AGREEMENT"). ANY USE, REPRODUCTION OR DISTRIBUTION OF THIS FILE
  * CONSTITUTES RECIPIENTS ACCEPTANCE OF THE AGREEMENT.
  *
  * You can obtain a current copy of the Eclipse Public License from
  * http://www.opensource.org/licenses/eclipse-1.0.php
  *
  * @author: org.sblim.wbemsmt.dcg.generator.DataContainerGenerator
  * @template: org/sblim/wbemsmt/dcg/templates/containerClass.vm
  *
  * Contributors: 
  *   Prashanth Karnam<prkarnam@in.ibm.com>
  * 
  * Description: 
  * 
  * generated Class
  */

package org.sblim.wbemsmt.firewall.bl.container.edit;

public interface FirewallManagedPortContainer extends org.sblim.wbemsmt.bl.adapter.DataContainer
{
	/** 
	 * Constants for Fields and Roles
	 **/
			public static final String FIELD_STARTPORT = "StartPort" ;
			public static final String FIELD_ENDPORT = "EndPort" ;
			public static final String FIELD_PROTOCOL = "Protocol" ;
				public static final String ROLE_INTERFACESFORPORT = "interfacesForPort";
	
	
	/** 
	 * Defined Fields
	 **/
			
		/**
		* <br>
		* DataType STRING<br>
		* UIType TEXTFIELD<br>
		* Relation to FCO: Linux_FirewallManagedPorts.StartPort<br>
		*/
		public org.sblim.wbemsmt.tools.input.LabeledBaseInputComponentIf get_StartPort();
			
		/**
		* <br>
		* DataType STRING<br>
		* UIType TEXTFIELD<br>
		* Relation to FCO: Linux_FirewallManagedPorts.EndPort<br>
		*/
		public org.sblim.wbemsmt.tools.input.LabeledBaseInputComponentIf get_EndPort();
			
		/**
		* <br>
		* DataType INTEGER<br>
		* UIType COMBOBOX<br>
		* Relation to FCO: Linux_FirewallManagedPorts.Protocol<br>
		*/
		public org.sblim.wbemsmt.tools.input.LabeledStringArrayInputComponentIf get_Protocol();
	
	
	/** 
	 * Linked DataContainers
	 **/
			
				
		/**
		* 
		* linked container FInterfacesForPortContainer
		*/

				
		public java.util.List<FInterfacesForPortContainer> getInterfacesForPort();
		
		
	   		
	   /**
		* Header for:
		* 
		* linked container FInterfacesForPortContainer
		*/
		public  org.sblim.wbemsmt.firewall.bl.container.edit.FInterfacesForPortContainerHeader getInterfacesForPortHeader();
		
			
}